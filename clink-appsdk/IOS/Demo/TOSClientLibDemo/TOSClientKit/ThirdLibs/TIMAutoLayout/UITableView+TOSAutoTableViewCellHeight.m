//
//  UITableView+TOSAutoTableViewCellHeight.m
//  TOSAutoLayout 测试 Demo
//
...
@interface TIMCellAutoHeightManager ()

@property (nonatomic, weak) UITableView *modelTableview;

@end

@implementation TIMCellAutoHeightManager
{
...
{
    TIMCellAutoHeightManager *manager = [[self alloc] initWithCellClass:cellClass tableView:tableView];
    return manager;
}
...
- (CGFloat)cellHeightForIndexPath:(NSIndexPath *)indexPath model:(id)model keyPath:(NSString *)keyPath
{
    ...
    if (cacheHeight) {
        ...
        if (self.modelCell.sd_indexPath && self.modelCell.sd_tableView) {
            ...
            [self.modelCell.contentView.autoLayoutModelsArray enumerateObjectsUsingBlock:^(TOSAutoLayoutModel *model, NSUInteger idx, BOOL *stop) {
...
@implementation UITableView (TOSAutoTableViewCellHeight)
...
- (CGFloat)cellHeightForIndexPath:(NSIndexPath *)indexPath model:(id)model keyPath:(NSString *)keyPath cellClass:(Class)cellClass contentViewWidth:(CGFloat)contentViewWidth
{
    if (!self.cellAutoHeightManager) {
        self.cellAutoHeightManager = [[TIMCellAutoHeightManager alloc] init];
...
- (TIMCellAutoHeightManager *)cellAutoHeightManager
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setCellAutoHeightManager:(TIMCellAutoHeightManager *)cellAutoHeightManager
{
...
@implementation UITableViewController (TIMTableViewControllerAutoCellHeight)
...
@implementation NSObject (TIMAnyObjectAutoCellHeight)
...
- (CGFloat)cellHeightForIndexPath:(NSIndexPath *)indexPath cellContentViewWidth:(CGFloat)width tableView:(UITableView *)tableView
{
    if (!tableView.cellAutoHeightManager) {
        tableView.cellAutoHeightManager = [[TIMCellAutoHeightManager alloc] init];

...
@interface TIMSYCacheFileManager ()
...
@implementation TIMSYCacheFileManager


+ (instancetype)shareManager
{
    static TIMSYCacheFileManager *fileManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        fileManager = [[TIMSYCacheFileManager alloc] init];
    });
    return fileManager;
}
...
{
    NSArray *array = [TIMSYCacheFileManager subFilesPathsWithFilePath:filePath];
    NSMutableArray *fileArray = [NSMutableArray arrayWithCapacity:array.count];
    for (id object in array) {
        TIMSYCacheFileModel *model = [TIMSYCacheFileModel new];
        model.fileName = object;
        model.filePath = [filePath stringByAppendingPathComponent:object];
        
        TIMSYCacheFileType type = [self fileTypeReadWithFilePath:model.filePath];
...
//                continue;
//            }
            if ([model.fileName hasPrefix:@"."] || ![TIMSYCacheFileManager isFileDirectoryWithFilePath:model.filePath]) {
                continue;
            }
        } else {
            // 过滤系统文件
            NSString *typeName = [TIMSYCacheFileManager fileTypeWithFilePath:model.filePath];
 ...
 */
- (TIMSYCacheFileType)fileTypeReadWithFilePath:(NSString *)filePath
{
    NSString *fileType = [TIMSYCacheFileManager fileTypeWithFilePath:filePath];
    fileType = fileType.lowercaseString;
    TIMSYCacheFileType type = TIMSYCacheFileTypeUnknow;
    if ([self.cacheVideoArrayUsing containsObject:fileType]) {
        type = TIMSYCacheFileTypeVideo;
    } else if ([self.cacheAudioArrayUsing containsObject:fileType]) {
        type = TIMSYCacheFileTypeAudio;
    } else if ([self.cacheImageArrayUsing containsObject:fileType]) {
        type = TIMSYCacheFileTypeImage;
    } else if ([self.cacheDocumentArrayUsing containsObject:fileType]) {
        type = TIMSYCacheFileTypeDocument;
    }
    return type;
}
...
- (UIImage *)fileTypeImageWithFilePath:(NSString *)filePath
{
    ...
    UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"folder_cacheFile"]];
    if (filePath && 0 < filePath.length) {
        TIMSYCacheFileType type = [self fileTypeReadWithFilePath:filePath];
        if (TIMSYCacheFileTypeUnknow == type) {
            image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"folder_cacheFile"]];
        } else {
            NSString *fileType = [TIMSYCacheFileManager fileTypeWithFilePath:filePath];
            fileType = fileType.lowercaseString;
            if ([self.cacheImageArrayUsing containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"image_cacheFile"]];
            } else if ([self.cacheVideoArrayUsing containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"video_cacheFile"]];
            } else if ([self.cacheAudioArrayUsing containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"audio_cacheFile"]];
            } else if ([@[@".doc", @".docx"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"doc_cacheFile"]];
            } else if ([@[@".xls", @".xlsx"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"xls_cacheFile"]];
            } else if ([@[@".pdf"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"pdf_cacheFile"]];
            } else if ([@[@".ppt", @".pptx"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"ppt_cacheFile"]];
            } else if ([@[@".zip", @".rar", @".iso"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"zip_cacheFile"]];
            } else if ([@[@".apk"] containsObject:fileType]) {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"apk_cacheFile"]];
            } else {
                image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"file_cacheFile"]];
            }
        }
    }
    return image;
...
+ (NSArray *)imagefilesWithFilePath:(NSString *)filePath
{
    ...
    [files enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        TIMSYCacheFileType type = [[TIMSYCacheFileManager shareManager] fileTypeReadWithFilePath:file];

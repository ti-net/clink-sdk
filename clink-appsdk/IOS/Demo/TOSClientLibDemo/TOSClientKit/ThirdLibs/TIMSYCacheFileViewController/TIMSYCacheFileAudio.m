...
@interface TIMSYCacheFileAudio () <AVAudioPlayerDelegate>
...
@implementation TIMSYCacheFileAudio


+ (instancetype)shareAudio
{
    static TIMSYCacheFileAudio *fileAudio;
    ...
    if (filePath && [filePath isKindOfClass:[NSString class]] && filePath.length > 0) {
        ...
        if (self.audioPlayer) {
            ...
            if (range.location != NSNotFound) {
                [[NSNotificationCenter defaultCenter] postNotificationName:TIMSYCacheFileAudioStopNotificationName object:nil];
                return;
            }
        }
...
- (void)fileAudioStop
{
    if (self.audioPlayer) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:TIMSYCacheFileAudioDurationValueChangeNotificationName object:number];
        //
        [self releaseSYCacheFileRead];
        [[NSNotificationCenter defaultCenter] postNotificationName:TIMSYCacheFileAudioStopNotificationName object:nil];
    }
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteAudioStop) name:TIMSYCacheFileAudioDeleteNotificationName object:nil];
}

...
#define sizeImage (timheightSYCacheDirectoryCell - originXY * 2)
#define frameImage (CGRectMake(originXY, originXY, (timheightSYCacheDirectoryCell - originXY * 2), (timheightSYCacheDirectoryCell - originXY * 2)))

#define widthScreen [UIScreen mainScreen].bounds.size.width

@interface TIMSYCacheFileCell ()
...
@implementation TIMSYCacheFileCell
...
- (void)setUI
{
    _backView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, widthScreen, timheightSYCacheDirectoryCell)];
    ...
    lineImage.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"line_cacheFile"]];
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetAudioProgress:) name:TIMSYCacheFileAudioDurationValueChangeNotificationName object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetAudioStop) name:TIMSYCacheFileAudioStopNotificationName object:nil];
}
...
{
    if (self.model.fileType == TIMSYCacheFileTypeAudio) {
...
- (void)setModel:(TIMSYCacheFileModel *)model
{
    _model = model;
    if (_model) {
        // 图标
        UIImage *image = [[TIMSYCacheFileManager shareManager] fileTypeImageWithFilePath:_model.filePath];
        ...
        TIMSYCacheFileType type = _model.fileType;
        if (type == TIMSYCacheFileTypeAudio) {

...
@implementation TIMSDImageCache {
    NSFileManager *_fileManager;
}

+ (TIMSDImageCache *)sharedImageCache {
...
#pragma mark TIMSDImageCache (private)
...
- (NSOperation *)queryDiskCacheForKey:(NSString *)key done:(SDWebImageQueryCompletedBlock)doneBlock {
    ...
    if (!key) {
        doneBlock(nil, TIMSDImageCacheTypeNone);
        return nil;
    }
    ...
    UIImage *image = [self imageFromMemoryCacheForKey:key];
    if (image) {
        doneBlock(image, TIMSDImageCacheTypeMemory);
        return nil;
    }
    ...
    dispatch_async(self.ioQueue, ^{
        ...
        @autoreleasepool {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                doneBlock(diskImage, TIMSDImageCacheTypeDisk);
            });
        }
    });

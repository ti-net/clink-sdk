...
@interface TIMSDWebImagePrefetcher ()

@property (strong, nonatomic) TIMSDWebImageManager *manager;
...
@implementation TIMSDWebImagePrefetcher

+ (TIMSDWebImagePrefetcher *)sharedImagePrefetcher {
...
- (id)init {
    if ((self = [super init])) {
        _manager = [TIMSDWebImageManager new];
...
- (void)startPrefetchingAtIndex:(NSUInteger)index {
    ...
    self.requestedCount++;
    [self.manager downloadImageWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, TIMSDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
        if (!finished) return;
        self.finishedCount++;

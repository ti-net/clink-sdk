...
@interface TIMRefreshBackNormalFooter()
{
...
@implementation TIMRefreshBackNormalFooter
#pragma mark - 懒加载子控件
- (UIImageView *)arrowView
{
    if (!_arrowView) {
        UIImageView *arrowView = [[UIImageView alloc] initWithImage:[NSBundle mj_arrowTIMImage]];
...
- (void)setState:(TIMRefreshState)state
{
    ...
    } else if (state == TIMRefreshStatePulling) {
        self.arrowView.hidden = NO;
        [self.loadingView stopAnimating];
        [UIView animateWithDuration:TIMRefreshFastAnimationDuration animations:^{
    ...
    } else if (state == TIMRefreshStateNoMoreData) {
        self.arrowView.hidden = YES;
        [self.loadingView stopAnimating];
    }
}

...
NSString * const TIMRefreshHeaderRefreshing2IdleBoundsKey = @"TIMRefreshHeaderRefreshing2IdleBounds";
NSString * const TIMRefreshHeaderRefreshingBoundsKey = @"TIMRefreshHeaderRefreshingBounds";

@interface TIMRefreshHeader() <CAAnimationDelegate>
@property (assign, nonatomic) CGFloat insetTDelta;
@end

@implementation TIMRefreshHeader
#pragma mark - 构造方法
+ (instancetype)headerWithRefreshingBlock:(TIMRefreshComponentAction)refreshingBlock
{
    TIMRefreshHeader *cmp = [[self alloc] init];
...
- (void)prepare
{
    ...
    // 设置key
    self.lastUpdatedTimeKey = TIMRefreshHeaderLastUpdatedTimeKey;
    
    // 设置高度
    self.mj_h = TIMRefreshHeaderHeight;
}
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.scrollView.isDragging) { // 如果正在拖拽
        ...
        if (self.state == TIMRefreshStateIdle && offsetY < normal2pullingOffsetY) {
            // 转为即将刷新状态
            self.state = TIMRefreshStatePulling;
        } else if (self.state == TIMRefreshStatePulling && offsetY >= normal2pullingOffsetY) {
            // 转为普通状态
...
- (void)setState:(TIMRefreshState)state
{
    ...
    // 根据状态做事情
    if (state == TIMRefreshStateIdle) {
        if (oldState != TIMRefreshStateRefreshing) return;
...
- (void)headerEndingAction {
    ...
    /**
     这个解决方法的思路出自 https://github.com/CoderMJLee/TIMRefresh/pull/844
     修改了用+ [UIView animateWithDuration: animations:]实现的修改contentInset的动画
     fix issue#225 https://github.com/CoderMJLee/TIMRefresh/issues/225
     另一种解法 pull#737 https://github.com/CoderMJLee/TIMRefresh/pull/737
    ...
    */
    ...
    boundsAnimation.duration = TIMRefreshSlowAnimationDuration;
    //在delegate里移除
    ...
    [boundsAnimation setValue:TIMRefreshHeaderRefreshing2IdleBoundsKey forKey:@"identity"];

    [self.scrollView.layer addAnimation:boundsAnimation forKey:TIMRefreshHeaderRefreshing2IdleBoundsKey];
    ...
    if (self.isAutomaticallyChangeAlpha) {
        ...
        opacityAnimation.duration = TIMRefreshSlowAnimationDuration;
        opacityAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        [self.layer addAnimation:opacityAnimation forKey:@"TIMRefreshHeaderRefreshing2IdleOpacity"];
...
- (void)headerRefreshingAction {
    ...
    if (self.scrollView.panGestureRecognizer.state != UIGestureRecognizerStateCancelled) {
        ...
        boundsAnimation.duration = TIMRefreshFastAnimationDuration;
        //在delegate里移除
        boundsAnimation.removedOnCompletion = NO;
        boundsAnimation.fillMode = kCAFillModeBoth;
        boundsAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        boundsAnimation.delegate = self;
        [boundsAnimation setValue:TIMRefreshHeaderRefreshingBoundsKey forKey:@"identity"];
        [self.scrollView.layer addAnimation:boundsAnimation forKey:TIMRefreshHeaderRefreshingBoundsKey];
    } else {
        [self executeRefreshingCallback];
    }
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    if ([identity isEqualToString:TIMRefreshHeaderRefreshing2IdleBoundsKey]) {
        self.pullingPercent = 0.0;
        self.scrollView.userInteractionEnabled = YES;
        if (self.endRefreshingCompletionBlock) {
            self.endRefreshingCompletionBlock();
        }
    } else if ([identity isEqualToString:TIMRefreshHeaderRefreshingBoundsKey]) {
    ...
    if ([self.scrollView.layer animationForKey:TIMRefreshHeaderRefreshing2IdleBoundsKey]) {
        [self.scrollView.layer removeAnimationForKey:TIMRefreshHeaderRefreshing2IdleBoundsKey];
    }
    
    if ([self.scrollView.layer animationForKey:TIMRefreshHeaderRefreshingBoundsKey]) {
        [self.scrollView.layer removeAnimationForKey:TIMRefreshHeaderRefreshingBoundsKey];
    }
}

...
#define KBottomMenuHeight 111.f  //底部菜单高度
#define KGridTopMargin 36.f  //顶部间距
#define KGridBottomMargin 20  //底部间距
#define KGridLRMargin 39.f   //左右边距
...
- (void)setupUI {
    ...
    self.maxGridRect = CGRectMake(KGridLRMargin, KGridTopMargin, self.view.sl_width - KGridLRMargin * 2, self.view.sl_height - KGridTopMargin - KGridBottomMargin - (self.view.sl_height - self.rotateBtn.sl_y));
    // self.view.sl_width - KGridLRMargin * 2
    //self.view.sl_height - KGridTopMargin - KGridBottomMargin- KBottomMenuHeight
...
- (UIButton *)rotateBtn {
    if (_rotateBtn == nil) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"SLEditImage_rotate"]];
        _rotateBtn = [[UIButton alloc] initWithFrame:CGRectMake(28.f, self.view.sl_height - KBottomMenuHeight - image.size.height, image.size.width, image.size.height)];
...
- (UIButton *)cancleClipBtn {
    if (_cancleClipBtn == nil) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"SLEditImage_EditImageClipCancel"]];
        _cancleClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(28.f, self.view.sl_height - 21.f - image.size.height, image.size.width, image.size.height)];
        [_cancleClipBtn setImage:image forState:UIControlStateNormal];
...
- (UIButton *)doneClipBtn {
    if (_doneClipBtn == nil) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"SLEditImage_EditImageClipDone"]];
        _doneClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.sl_width - 30 - image.size.width, self.view.sl_height - 21.f - image.size.height, image.size.width, image.size.height)];
        [_doneClipBtn setImage:image forState:UIControlStateNormal];
...
- (void)rotateBtnClicked:(id)sender {
    ...
    switch (_rotateAngle) {
        case 90:    angleInRadians = (M_PI + M_PI_2);            break;
        case -90:  angleInRadians = -(M_PI + M_PI_2);           break;
        case 180:   angleInRadians = -M_PI;              break;
        case -180: angleInRadians = M_PI;             break;
        case 270:   angleInRadians = M_PI_2;   break;
        case -270: angleInRadians = -M_PI_2;  break;
        default:                                        break;
    }
    //旋转前获得网格框在图片上选择的区域

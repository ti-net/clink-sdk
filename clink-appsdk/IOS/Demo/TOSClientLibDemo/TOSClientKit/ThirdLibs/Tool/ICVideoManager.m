...
//        [formatter setDateFormat:@"yyyy-MM-ddHH:mm:ss"];
        NSString *videoName = [kitUtils getMsgUUID]; //[formatter stringFromDate:[NSDate date]];
         NSString *outPutPath = [self videoPathWithFileName:videoName];
//        NSLog(@"导出的路径 outPutPath = %@",outPutPath);
...
//                    NSLog(@"导出 完成 outPutPath = %@",outPutPath);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self tim_showMBErrorView:@"视频暂时无法加载，请在相册中加载后上传"];
                });
            }
        }];
        return outPutPath;
...
- (NSString *)WriteDataToFile:(NSString *)fileName data:(NSData*)fileData{
    NSString *outPutPath = [self videoPathWithFileName:fileName];
    [[NSFileManager defaultManager] createFileAtPath:outPutPath contents:fileData attributes:nil];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    //查找文件，如果没有就创建一个文件
    if (![fileManager fileExistsAtPath: outPutPath]) {
        BOOL isSuccess = [fileManager createFileAtPath:outPutPath contents:nil attributes:nil];
        TIMKitLog(@"isSiccess = %d",isSuccess);
    }
    NSError *error = nil;
    BOOL isSuccess = [fileData writeToFile:outPutPath options:NSDataWritingAtomic error:&error];
    if (isSuccess && error == nil) {
        TIMKitLog(@"存储成功！！！");
    }else{
        TIMKitLog(@"error = %@",error);
        TIMKitLog(@"存储失败！！！");
    }
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            switch ([exportSession status]) {
                case AVAssetExportSessionStatusFailed: {
                    TIMKitLog(@"failed, error:%@.", exportSession.error);
                } break;

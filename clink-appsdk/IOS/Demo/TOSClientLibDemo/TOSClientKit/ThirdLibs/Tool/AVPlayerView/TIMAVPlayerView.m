...
- (void)addTimeObserver {
    ...
    self.timeObserverToken = [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if ((CGFloat)CMTimeGetSeconds(time)<= [strongSelf getTotalPlayTime]) {
            /// 更新进度条
            strongSelf.progressView.value = [strongSelf getCurrentPlayTime];
        }
        else {
            strongSelf.progressView.value = [strongSelf getTotalPlayTime];
        }
...
- (UIButton *)playButton {
    if (!_playButton) {
        _playButton = [[UIButton alloc] initWithFrame:(CGRectMake(10, self.bounds.size.height - 100, 40, 40))];
        [_playButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", FRAMEWORKS_BUNDLE_PATH, @"videoPause"]] forState:(UIControlStateNormal)];
        [_playButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", FRAMEWORKS_BUNDLE_PATH, @"videoPlay"]] forState:(UIControlStateSelected)];
...
- (UISlider *)progressView {
    if (!_progressView) {
        _progressView = [[UISlider alloc] initWithFrame:(CGRectMake(60, self.bounds.size.height - 95, self.bounds.size.width-110, 30))];

...
CGFloat const TIMMBProgressMaxOffset = 1000000.f;
...
@interface TIMMBProgressHUDRoundedButton : UIButton
@end


@implementation TIMMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    TIMMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    TIMMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (TIMMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            TIMMBProgressHUD *hud = (TIMMBProgressHUD *)subview;
...
- (void)setupViews {
    ...
    TIMMBBackgroundView *backgroundView = [[TIMMBBackgroundView alloc] initWithFrame:self.bounds];
    ...
    TIMMBBackgroundView *bezelView = [TIMMBBackgroundView new];
    ...
    UIButton *button = [TIMMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[TIMMBRoundProgressView class]];
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[TIMMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[TIMMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MBProgressHUDModeAnnularDeterminate) {
            [(TIMMBRoundProgressView *)indicator setAnnular:YES];
        }
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[TIMMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[TIMMBProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[TIMMBRoundProgressView class]]) {
        TIMMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TIMMBRoundProgressView appearanceWhenContainedIn:[TIMMBProgressHUD class], nil];
#else
        appearance = [TIMMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[TIMMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((TIMMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((TIMMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[TIMMBBarProgressView class]]) {
        TIMMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TIMMBBarProgressView appearanceWhenContainedIn:[TIMMBProgressHUD class], nil];
#else
        appearance = [TIMMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[TIMMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((TIMMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((TIMMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    TIMMBBackgroundView *bezelView = self.bezelView;
...
@implementation TIMMBRoundProgressView
...
@implementation TIMMBBarProgressView
...
@implementation TIMMBBackgroundView
...
@implementation TIMMBProgressHUDRoundedButton

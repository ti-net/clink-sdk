...
YYSYNTH_DUMMY_CLASS(UIFont_TIMYYAdd)
...
@implementation UIFont (TIMYYAdd)
...
static uint32_t TIMCalcTableCheckSum(const uint32_t *table, uint32_t numberOfBytesInTable) {
    uint32_t sum = 0;
...
+ (NSData *)dataFromCGFont:(CGFontRef)cgFont {
    ...
    size_t totalSize = sizeof(TIMFontHeader) + sizeof(TIMTableEntry) * tableCount;
    ...
    TIMFontHeader *offsetTable = (TIMFontHeader *)dataPtr;
    ...
    dataPtr += sizeof(TIMFontHeader);
    
    // write tables
    TIMTableEntry *entry = (TIMTableEntry *)dataPtr;
    dataPtr += sizeof(TIMTableEntry) * tableCount;
    ...
    for (int index = 0; index < tableCount; ++index) {
        ...
        entry->fCheckSum = CFSwapInt32HostToBig(TIMCalcTableCheckSum((uint32_t *)dataPtr, (uint32_t)tableSize));

...
/**
 TIMYYKVStorageItem is used by `YYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface TIMYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `TIMYYKVStorageItem.value` stored.
 ...
 * If you want to store large files (such as image cache),
   use TIMYYKVStorageTypeFile to get better performance.
 * You can use TIMYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, TIMYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    TIMYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    TIMYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    TIMYYKVStorageTypeMixed = 2,
};
 ...
 */
@interface TIMYYKVStorage : NSObject
...
@property (nonatomic, readonly) TIMYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(TIMYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is TIMYYKVStorageTypeFile, then the item.filename should not be empty.
 ...
 */
- (BOOL)saveItem:(TIMYYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 TIMYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is TIMYYKVStorageTypeFile, then the `filename` should not be empty.
 ...
 */
- (nullable TIMYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable TIMYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `TIMYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TIMYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `TIMYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TIMYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;

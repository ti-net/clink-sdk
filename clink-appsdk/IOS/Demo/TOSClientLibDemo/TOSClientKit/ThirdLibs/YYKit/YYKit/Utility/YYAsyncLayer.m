...
- (void)_displayAsync:(BOOL)async {
    if (self.bounds.size.width<=0 || self.bounds.size.height<=0 || isnan(self.bounds.size.width) || isnan(self.bounds.size.height)) {
        self.contents = nil;
        return;
    }
...
//            CGFloat scale = self.contentsScale;  // 默认值 1.0f
            UIImage *image = nil;

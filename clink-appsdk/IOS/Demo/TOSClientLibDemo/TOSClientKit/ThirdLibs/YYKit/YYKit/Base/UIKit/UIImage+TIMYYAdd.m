...
static NSTimeInterval _timyy_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
@implementation UIImage (TIMYYAdd)
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _timyy_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)imageWithColor:(UIColor *)color size:(CGSize)size {
    if (!color || size.width <= 0 || size.height <= 0 || isnan(size.width) || isnan(size.height)) return nil;
...
+ (UIImage *)imageWithSize:(CGSize)size drawBlock:(void (^)(CGContextRef context))drawBlock {
    ...
    if (size.width <= 0 || size.height <= 0 || isnan(size.width) || isnan(size.height)) return nil;
...
- (UIImage *)imageByRoundCornerRadius:(CGFloat)radius
    ...
    if (self.size.width <= 0 || self.size.height <= 0 || isnan(self.size.width) || isnan(self.size.height)) return nil;

...
static int64_t _TIMYYDiskSpaceFree() {
    NSError *error = nil;
...
static void _TIMYYDiskCacheInitGlobal() {
...
static TIMYYDiskCache *_TIMYYDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _TIMYYDiskCacheInitGlobal();
...
static void _TIMYYDiskCacheSetGlobal(TIMYYDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _TIMYYDiskSpaceFree();
...
- (instancetype)initWithPath:(NSString *)path
    ...
    TIMYYDiskCache *globalCache = _TIMYYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    TIMYYKVStorageType type;
    if (threshold == 0) {
        type = TIMYYKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = TIMYYKVStorageTypeSQLite;
    } else {
        type = TIMYYKVStorageTypeMixed;
    }
    
    TIMYYKVStorage *kv = [[TIMYYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    TIMYYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [TIMYYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [TIMYYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != TIMYYKVStorageTypeSQLite) {

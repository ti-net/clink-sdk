...
/**
 TIMYYMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 TIMYYMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in TIMYYMemoryCache is typically in constant time (O(1)).
 */
@interface TIMYYMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(TIMYYMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(TIMYYMemoryCache *cache);

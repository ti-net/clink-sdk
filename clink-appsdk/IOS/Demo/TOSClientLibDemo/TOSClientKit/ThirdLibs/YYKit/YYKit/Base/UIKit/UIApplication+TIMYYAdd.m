...
YYSYNTH_DUMMY_CLASS(UIApplication_TIMYYAdd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _TIMYYUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _TIMYYUIApplicationNetworkIndicatorInfo
@end


@implementation UIApplication (TIMYYAdd)
...
YYSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _TIMYYUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _TIMYYUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_TIMYYUIApplicationNetworkIndicatorInfo new];

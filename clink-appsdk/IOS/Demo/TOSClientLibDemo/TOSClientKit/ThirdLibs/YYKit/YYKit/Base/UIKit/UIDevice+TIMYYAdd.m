...
@implementation UIDevice (TIMYYAdd)
...
static uint64_t yy_net_counter_get_by_type(yy_net_interface_counter *counter, TIMYYNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & TIMYYNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & TIMYYNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & TIMYYNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & TIMYYNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & TIMYYNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & TIMYYNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

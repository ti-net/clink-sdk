...
@implementation TIMQEmotionHelper

+ (TIMQEmotionHelper *)sharedEmotionHelper {
    static TIMQEmotionHelper *_sharedFaceManager = nil;
    static dispatch_once_t predUser;
    dispatch_once(&predUser, ^{
        _sharedFaceManager = [[TIMQEmotionHelper alloc] init];
...
- (void)setEmotionArray:(NSArray<TIMQEmotion *> *)emotionArray {
...
- (NSDictionary<NSString *, UIImage *> *)cacheTotalImageDictionary {
    if (!_cacheTotalImageDictionary) {
        ...
        for (TIMQEmotion *e in _emotionArray) {
            if (!e.image) {
...
- (NSAttributedString *)obtainAttributedStringByImageKey:(NSString *)imageKey font:(UIFont *)font useCache:(BOOL)useCache {
    ...
    if (!useCache) {
        ...
        TIMQEmotionAttachment *attachMent = [[TIMQEmotionAttachment alloc] init];

...
typedef NS_ENUM(NSUInteger, TIMInputState) {
    TIMInputStateNormal = 0, //默认状态，没弹出键盘 也没弹出表情 也没弹出extend面板
    TIMInputStateText,//弹出软键盘状态
    TIMInputStateEmotion,//弹出表情状态
    TIMInputStateExtend,//弹出extend面板状态
};

@interface TIMQKeyboardManager()

//true：输入条平时是不显示出来的；false：输入条一直都在vc底部
@property (nonatomic, assign) BOOL inputBarBelowViewController;

//当前的输入状态
@property (nonatomic, assign) TIMInputState currentInputState;
...
@implementation TIMQKeyboardManager
...
- (void)addBottomInputBarView:(UIView *)inputBarView belowViewController:(BOOL)belowViewController {
    ...
    [self callBackWholeInputViewHeightDidChange:belowViewController ? 0 : inputBarView.frame.size.height reason:TIMWholeInputViewHeightDidChangeReasonWillAddToSuperView];
...
- (void)onKeyboardWillShowOrHideByNotifications:(CGRect)keyboardRect animationOptions:(UIViewAnimationOptions)animationOptions duration:(double)duration showKeyboard:(BOOL)showKeyboard {
    ...
    TIMInputState previousInputState = self.currentInputState;
    
    if (showKeyboard) {
        //马上要显示软键盘了，设置当前state为Text模式。
        self.currentInputState = TIMInputStateText;
    }
    ...
    if (self.currentInputState == TIMInputStateText) {
        ...
        [UIView animateWithDuration:duration
            ...
            if (showKeyboard){
                ...
                switch (previousInputState) {
                    case TIMInputStateEmotion: {
                        [self switchEmotionBoardView:YES];
                        break;
                    }
                    case TIMInputStateExtend: {
            ...
            [self callBackWholeInputViewHeightDidChange:self.viewController.view.frame.size.height
                              - self.inputBarView.frame.origin.y - self.safeAreaInsetsBottom reason:showKeyboard ? TIMWholeInputViewHeightDidChangeReasonKeyboardWillShow : TIMWholeInputViewHeightDidChangeReasonKeyboardWillHide];
...
- (void)hideKeyboardAndSwitchToCurrentBoardView:(BOOL)allBoardHide {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        //回调给ViewController
        [self callBackWholeInputViewHeightDidChange:self.viewController.view.frame.size.height
         - self.inputBarView.frame.origin.y - self.safeAreaInsetsBottom reason:allBoardHide ? TIMWholeInputViewHeightDidChangeReasonBoardViewDidHide : TIMWholeInputViewHeightDidChangeReasonBoardViewDidShow];
        
    } completion:^(BOOL finished) {
        if (allBoardHide) {
            self.currentInputState = TIMInputStateNormal;
        }
    }];
}
...
//private
- (void)callBackWholeInputViewHeightDidChange:(CGFloat)wholeInputViewHeight reason:(TIMWholeInputViewHeightDidChangeReason)reason {
...
- (void)inputTextViewHeightDidChange:(BOOL)becauseSendText {
    //回调给ViewController
    [self callBackWholeInputViewHeightDidChange: self.viewController.view.frame.size.height
     - self.inputBarView.frame.origin.y - _safeAreaInsetsBottom reason: becauseSendText ? TIMWholeInputViewHeightDidChangeReasonTextDidSend : TIMWholeInputViewHeightDidChangeReasonTextDidChange];
}

// public - 隐藏所有面板，包括表情面板和拓展面板 和 软键盘
- (void)hideAllBoardView {
    if (self.currentInputState != TIMInputStateNormal) {
...
- (void)switchToEmotionBoardKeyboard {
    ...
    self.currentInputState = TIMInputStateEmotion;
...
- (void)switchToExtendBoardKeyboard {
    ...
    self.currentInputState = TIMInputStateExtend;

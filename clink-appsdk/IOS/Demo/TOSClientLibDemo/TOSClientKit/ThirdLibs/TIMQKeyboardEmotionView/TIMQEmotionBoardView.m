...
//  Copyright © 2016年 侯力 . All rights reserved. 作者DQ：这里的代码我是从QMUI里摘抄出来再做了删减。然后我修改了修复QMUI的两个BUG，1：半透明表情的Rect计算方式有问题，2：重新优化点击后的灰色背景的大小改为计算，而不是设置死，3：添加了顶部的一条细线
//
...
@class TIMUIEmotionPageView;

@protocol TIMUIEmotionPageViewDelegate <NSObject>

@optional
- (void)emotionPageView:(TIMUIEmotionPageView *)emotionPageView didSelectEmotion:(TIMQEmotion *)emotion atIndex:(NSInteger)index;
- (void)emotionPageViewDidLayoutEmotions:(TIMUIEmotionPageView *)emotionPageView;
@end

/// 表情面板每一页的cell，在drawRect里将所有表情绘制上去，同时自带一个末尾的删除按钮
@interface TIMUIEmotionPageView : UIView

@property(nonatomic, weak) TIMQEmotionBoardView<TIMUIEmotionPageViewDelegate> *delegate;
...
@implementation TIMUIEmotionPageView
...
- (void)setEmotions:(NSArray<TIMQEmotion *> *)emotions {
...
- (void)handleTapGestureRecognizer:(UITapGestureRecognizer *)gestureRecognizer {
    ...
    for (NSInteger i = 0; i < self.emotionHittingRects.count; i ++) {
        ...
        if (CGRectContainsPoint(rect, location)) {
            ...
            TIMQEmotion *emotion = self.emotions[i];
...
@property(nonatomic, strong) TIMUIEmotionPageView *pageView;
@end
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        _pageView = [[TIMUIEmotionPageView alloc] init];
...
- (void)setEmotions:(NSArray<TIMQEmotion *> *)emotions
   ...
   emotionSelectedBackgroundExtension:(UIEdgeInsets)emotionSelectedBackgroundExtension
    ...
    TIMUIEmotionPageView *pageView = self.pageView;
...
@interface TIMQEmotionBoardView ()<TIMUIEmotionPageViewDelegate ,UIScrollViewDelegate>
/// 用于展示表情面板的竖向滚动 scrollView，布局撑满整个控件
@property(nonatomic, strong, readonly) UIEmotionVerticalScrollView *verticalScrollView;
@property(nonatomic, strong) NSMutableArray<NSArray<TIMQEmotion *> *> *pagedEmotions;
@property(nonatomic, assign) BOOL debug;
@end

@implementation TIMQEmotionBoardView
...
@interface TIMQEmotionBoardView (UIAppearance)

@end

@implementation TIMQEmotionBoardView (UIAppearance)
...
+ (void)setDefaultAppearance {
    ...
    NSBundle *bundle = [NSBundle tos_tz_imagePickerBundle];//[NSBundle bundleForClass:[QEmotionBoardView class]];
    
    TIMQEmotionBoardView *appearance = [TIMQEmotionBoardView appearance];
//    appearance.backgroundColor = TOSHexColor(0xF3F6F9);
    ...
    UIPageControl *pageControlAppearance = [UIPageControl appearanceWhenContainedInInstancesOfClasses:@[[TIMQEmotionBoardView class]]];

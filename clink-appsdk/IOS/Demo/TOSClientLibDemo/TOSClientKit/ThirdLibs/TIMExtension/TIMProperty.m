...
@interface TIMProperty()
@property (strong, nonatomic) NSMutableDictionary *propertyKeysDict;
@property (strong, nonatomic) NSMutableDictionary *objectClassInArrayDict;
@end

@implementation TIMProperty
...
{
    TIMProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    TIMExtensionAssertParamNotNil(property);
    ...
    _type = [TIMPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                TIMPropertyKey *propertyKey = [[TIMPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                TIMPropertyKey *subPropertyKey = [[TIMPropertyKey alloc] init];
                subPropertyKey.type = TIMPropertyKeyTypeArray;

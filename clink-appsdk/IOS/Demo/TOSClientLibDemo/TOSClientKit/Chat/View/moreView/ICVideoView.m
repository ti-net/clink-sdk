...
//    self.recordBtn.frame = CGRectMake(0, self.height-20-70, 70, 70);
    ...
    [exitBtn mas_TIMmakeTIMConstraints:^(TIMMASConstraintMaker *make) {
        make.right.equalTo(self.mas_TIMright).offset(-50);
        make.centerY.equalTo(self->_recordBtn.mas_TIMcenterY);
        make.width.mas_TIMequalTo(40);
        make.height.mas_TIMequalTo(40);
    }];
    ...
    label.text = @"正在加载...";
    [label mas_TIMmakeTIMConstraints:^(TIMMASConstraintMaker *make) {
        make.centerX.equalTo(self->_videoLayerView.mas_TIMcenterX);
        make.centerY.equalTo(self->_videoLayerView.mas_TIMcenterY);
    }];
...
- (void)recordVideoStarted
{
    ...
    self.recordTimer = [NSTimer scheduledTimerWithTimeInterval:20.5 target:self selector:@selector(recordTimeOver) userInfo:nil repeats:NO];
...
- (void)recordVideoCanceled
{
    ...
    self.promptLabel.hidden = YES;
    [[ICVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
        self->_recordBtn.hidden = NO;
        // 删除已经录制的文件
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[ICVideoManager shareManager] cancelRecordingVideoWithFileName:self->_videoName];
        });
    }];
}

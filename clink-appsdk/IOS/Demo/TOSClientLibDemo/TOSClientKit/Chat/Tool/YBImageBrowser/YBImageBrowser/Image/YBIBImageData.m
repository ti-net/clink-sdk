...
extern CGImageRef TIMCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
...
- (void)loadURL_queryCache {
    ...
    [self.yb_webImageMediator() yb_queryCacheOperationForKey:self.imageURLKey completed:^(UIImage * _Nullable image, NSData * _Nullable imageData) {
...
//                __strong typeof(wSelf) self = wSelf;
//                if (!self) return;
//                [self.yb_webImageMediator() yb_storeToDiskWithImageData:imageData forKey:self.imageURLKey];
//                self.loadingStatus = YBIBImageLoadingStatusNone;
//                if (image) {
//                    [self setOriginImageAndLoadWithImage:image];
//                } else {
...
- (void)yb_saveToPhotoAlbum {
    ...
    [YBIBPhotoAlbumManager getPhotoAlbumAuthorizationSuccess:^{
        if ([self.originImage conformsToProtocol:@protocol(YYAnimatedImage)] && [self.originImage respondsToSelector:@selector(animatedImageData)] && [self.originImage performSelector:@selector(animatedImageData)]) {
            ...
            [self.yb_webImageMediator() yb_queryCacheOperationForKey:self.imageURLKey completed:^(UIImage * _Nullable image, NSData * _Nullable data) {
                if (data) {
                    saveData(data);
                } else if (image) {
                    saveImage(image);

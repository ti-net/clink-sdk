...
+ (TIMMessageFrame *)createMessageFrame:(NSString *)type
    ...
    return  [ICMessageHelper createMessageFrame:type originalType:originalType content:content extra:extra auditExtra:auditExtra path:path urlPath:urlPath from:from to:to fileKey:fileKey bigImgFileId:bigImgFileId voiceDuration:voiceDuration msgId:msgId sendTime:sendTime showTime:showTime picSize:size picType:picType isSender:isSender receivedSenderByYourself:receivedSenderByYourself status:status fullJSONString:nil senderType:senderType];
}
                             ...
                             senderType:(NSNumber *)senderType
{
    ...
    if ([type isEqualToString:TypeText]) {
        ...
        message.content = @"[图片]";
        NSDictionary *dic = [kitUtils dictionaryWithJsonString:jsonStr];

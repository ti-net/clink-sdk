...
- (void)createSubmenu {
    ...
    NSArray *colors = @[TOSHexColor(0xFFFFFF), TOSHexColor(0x2B2B2B), TOSHexColor(0xFD524F), TOSHexColor(0xFFC300), TOSHexColor(0x06C25F), TOSHexColor(0x11AEFE), TOSHexColor(0x6468F0), [UIColor clearColor]];
    int count = (int)colors.count;
    CGSize itemSize = CGSizeMake(24.f, 24.f);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == count - 1) {
            ...
            [colorBtn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,@"SLEditImage_EditMenuGraffitiBack"]] forState:UIControlStateNormal];
        }else {
...
- (void)createSubmenu {
    ...
    NSArray *imageNames = @[@"EditMenuMosaic", @"EditBrushMosaic", @"SLEditImage_EditMenuGraffitiBack"];
    NSArray *imageNamesSelected = @[@"EditMenuMosaicSelected", @"EditBrushMosaicSelected", @"SLEditImage_EditMenuGraffitiBack"];
...
- (void)createEditMenus {
    ...
    CGSize itemSize = CGSizeMake(21.f, 21.f);
    CGFloat space = 32.f;
    _menuBtns = [NSMutableArray array];
    for (int i = 0; i < count; i++) {
        UIButton * menuBtn = [[UIButton alloc] initWithFrame:CGRectMake(23.f + (itemSize.width + space)*i, self.sl_height - 32.f - itemSize.height, itemSize.width, itemSize.height)];
        menuBtn.tag = [_menuTypes[i] intValue];
        
        [menuBtn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,_imageNames[i]]] forState:UIControlStateNormal];
        [menuBtn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@/%@",FRAMEWORKS_BUNDLE_PATH,_imageNamesSelected[i]]] forState:UIControlStateSelected];
...
- (void)setEditObject:(SLEditObject)editObject {
    ...
    if (editObject == SLEditObjectPicture) {
        _menuTypes = @[@(SLEditMenuTypeGraffiti), @(SLEditMenuTypeText), @(SLEditMenuTypePictureClipping)];
        _imageNames = @[@"SLEditImage_EditMenuGraffiti", @"SLEditImage_EditMenuText", @"SLEditImage_EditMenuClipImage"];
        _imageNamesSelected = @[@"SLEditImage_EditMenuGraffitiSelected", @"SLEditImage_EditMenuText",@"SLEditImage_EditMenuClipImage"];
        ...
        _imageNames = @[@"SLEditImage_EditMenuGraffiti", @"EditMenuSticker", @"SLEditImage_EditMenuText", @"EditMenuCut"];
        _imageNamesSelected = @[@"SLEditImage_EditMenuGraffitiSelected", @"EditMenuStickerSelected", @"SLEditImage_EditMenuText", @"EditMenuCut"];
    }
...
- (SLSubmenuGraffitiView *)submenuGraffiti {
    if (!_submenuGraffiti) {
        _submenuGraffiti = [[SLSubmenuGraffitiView alloc] initWithFrame:CGRectMake(0, self.sl_height - 81.f - 24.f, SL_kScreenWidth, 24.f)];

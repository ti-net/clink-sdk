...
- (instancetype)init {
    ...
    if (self) {
        ...
        _gridColor = TOSHexColor(0xD8D8D8);
...
- (CGPathRef)drawGrid {
    ...
    CGFloat offset = 1;
    /** 长度 */
    CGFloat cornerlength = 16.f;
    
    for (NSInteger i = 0; i < 3; i++) {
        CGRect newRct = CGRectInset(rct, -offset-i, -offset-i);
        [self pathLineDrawingWithPath:path newRct:newRct cornerlength:cornerlength+i];
    }
    return path.CGPath;
}

- (void)pathLineDrawingWithPath:(UIBezierPath *)path  newRct:(CGRect)newRct cornerlength:(CGFloat)cornerlength {
    /** 左上角 */
...
// 网格四角和边的 可控范围
const CGFloat kSLControlWidth = 16.f;
//网格视图
...
- (SLGridLayer *)gridLayer {
    if (!_gridLayer) {
        ...
        _gridLayer.gridColor = TOSHexColor(0xD8D8D8);

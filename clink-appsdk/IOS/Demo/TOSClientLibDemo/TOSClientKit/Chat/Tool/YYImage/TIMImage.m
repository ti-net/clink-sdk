...
+ (TIMImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (TIMImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (TIMImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        TIMImageDecoder *decoder = [TIMImageDecoder decoderWithData:data scale:scale];
        TIMImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];

 ...
 */
@interface _TIMImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _TIMImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_TIMImageWeakProxy alloc] initWithTarget:target];
}
...
- (void)main {
    __strong TIMAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation TIMAnimatedImageView
...
- (void)resetAnimated {
    if (!_link) {
        ...
        _link = [CADisplayLink displayLinkWithTarget:[_TIMImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)step:(CADisplayLink *)link {
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _TIMAnimatedImageViewFetchOperation *operation = [_TIMAnimatedImageViewFetchOperation new];
        operation.view = self;

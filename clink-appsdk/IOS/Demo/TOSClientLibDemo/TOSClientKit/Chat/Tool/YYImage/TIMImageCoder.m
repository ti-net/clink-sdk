...
CGColorSpaceRef TIMCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef TIMCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL TIMCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, TIMCGColorSpaceGetDeviceRGB());
}

BOOL TIMCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, TIMCGColorSpaceGetDeviceGray());
}
...
static BOOL YYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = TIMCGColorSpaceGetDeviceRGB();
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, TIMCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef TIMCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, TIMCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef TIMCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, TIMCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, TIMCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation TIMUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger TIMUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef TIMCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    return TIMCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

YYImageType TIMImageDetectType(CFDataRef data) {
...
CFStringRef TIMImageTypeToUTType(YYImageType type) {
    switch (type) {
...
YYImageType TIMImageTypeFromUTType(CFStringRef uti) {
...
NSString *TIMImageTypeGetExtension(YYImageType type) {
    switch (type) {
...
CFDataRef TIMCGImageCreateEncodedData(CGImageRef imageRef, YYImageType type, CGFloat quality) {
    ...
    if (type == YYImageTypeWebP) {
        ...
        if (quality == 1) {
            return TIMCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, YYImagePresetDefault);
        } else {
            return TIMCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, YYImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = TIMImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL TIMImageWebPAvailable() {
    return YES;
}

CFDataRef TIMCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, YYImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
...
NSUInteger TIMImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef TIMCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, TIMCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation TIMImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    TIMImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    TIMImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _TIMImageDecoderFrame : TIMImageFrame
...
@implementation _TIMImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _TIMImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation TIMImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    TIMImageDecoder *decoder = [[TIMImageDecoder alloc] initWithScale:scale];
...
- (TIMImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    TIMImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_TIMImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    YYImageType type = TIMImageDetectType((__bridge CFDataRef)data);
...
- (TIMImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _TIMImageDecoderFrame *frame = [(_TIMImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = TIMCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _TIMImageDecoderFrame *frame = [_TIMImageDecoderFrame new];
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = TIMUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _TIMImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = TIMCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TIMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TIMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, TIMCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, TIMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_TIMImageDecoderFrame *)frame {
...
- (CGImageRef)_newBlendedImageWithFrame:(_TIMImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == YYImageDisposePrevious) {
        if (frame.blend == YYImageBlendOver) {
...
@implementation TIMImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"TIMImageEncoder init error" reason:@"TIMImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, TIMImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, TIMImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
        ...
        @autoreleasepool {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = TIMCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return TIMCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return TIMCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = TIMCGImageCreateEncodedData(decoded, YYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, TIMCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = TIMCGImageCreateEncodedData(extendedImage, YYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = TIMCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, YYImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
+ (NSData *)encodeImage:(UIImage *)image type:(YYImageType)type quality:(CGFloat)quality {
    TIMImageEncoder *encoder = [[TIMImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(TIMImageDecoder *)decoder type:(YYImageType)type quality:(CGFloat)quality {
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[TIMSpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[TIMImage class]]) {
        TIMImage *image = (id)self;
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = TIMCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }

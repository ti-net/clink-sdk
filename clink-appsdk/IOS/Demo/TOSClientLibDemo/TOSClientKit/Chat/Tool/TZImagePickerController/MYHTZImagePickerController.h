...
/*
 经过测试，比起xib的方式，把MYHTZAssetCell改用纯代码的方式来写，滑动帧数明显提高了（约提高10帧左右）
...
@class MYHTZAlbumCell, MYHTZAssetCell;
@protocol MYHTZImagePickerControllerDelegate;
@interface MYHTZImagePickerController : UINavigationController
...
@property (nonatomic, strong) NSMutableArray<MYHTZAssetModel *> *selectedModels;
...
@property (nonatomic, weak) id<MYHTZImagePickerControllerDelegate> pickerDelegate;

@end


@protocol MYHTZImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these callback will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
// You can get original photo with asset, by the method [[MYHTZImageManager manager] getOriginalPhotoWithAsset:completion:].
// The UIImage Object in photos default width is 828px, you can set it by photoWidth property.
// 这个照片选择器会自己dismiss，当选择器dismiss的时候，会执行下面的代理方法
// 你也可以设置autoDismiss属性为NO，选择器就不会自己dismis了
// 如果isSelectOriginalPhoto为YES，表明用户选择了原图
// 你可以通过一个asset获得原图，通过这个方法：[[MYHTZImageManager manager] getOriginalPhotoWithAsset:completion:]
...
- (void)imagePickerController:(MYHTZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
...
// 如果allowPickingMultipleVideo是YES，将会调用imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:
- (void)imagePickerController:(MYHTZImagePickerController *)picker didFinishPickingGifImage:(UIImage *)animatedImage sourceAssets:(PHAsset *)asset;
...
@interface MYHTZAlbumPickerController : UIViewController
...
@property (nonatomic, copy) void (^gifImagePlayBlock)(MYHTZPhotoPreviewView *view, UIImageView *imageView, NSData *gifData, NSDictionary *info);
@end

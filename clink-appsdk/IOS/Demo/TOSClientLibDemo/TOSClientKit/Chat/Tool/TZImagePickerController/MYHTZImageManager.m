...
@interface MYHTZImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation MYHTZImageManager

CGSize TIMAssetGridThumbnailSize;
CGFloat TIMTZScreenWidth;
CGFloat TIMTZScreenScale;

static MYHTZImageManager *manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (TIMTZScreenWidth - 2 * margin - 4) / columnNumber - margin;
    TIMAssetGridThumbnailSize = CGSizeMake(itemWH * TIMTZScreenScale, itemWH * TIMTZScreenScale);
}

- (void)configTZScreenWidth {
    TIMTZScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    TIMTZScreenScale = 2.0;
    if (TIMTZScreenWidth > 700) {
        TIMTZScreenScale = 1.5;
    }
}
...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(MYHTZAlbumModel *model))completion {
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbumWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(MYHTZAlbumModel *model))completion {
    __block MYHTZAlbumModel *model;
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
    ...
    /// 兼容iOS18需要把 PHAssetCollectionSubtypeAlbumRegular 枚举改成 PHAssetCollectionSubtypeAny 枚举
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<MYHTZAlbumModel *> *))completion {
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAllAlbumsWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<MYHTZAlbumModel *> *))completion {
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<MYHTZAssetModel *> *))completion {
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<MYHTZAssetModel *> *))completion {
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        MYHTZAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:config.allowPickingVideo allowPickingImage:config.allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(MYHTZAssetModel *))completion {
    TZImagePickerConfig *config = [TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index completion:(void (^)(MYHTZAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (MYHTZAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    MYHTZAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == MYHTZAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == MYHTZAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == MYHTZAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == MYHTZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [MYHTZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (MYHTZAssetModelMediaType)getAssetType:(PHAsset *)asset {
    MYHTZAssetModelMediaType type = MYHTZAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = MYHTZAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = MYHTZAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = MYHTZAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = MYHTZAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        MYHTZAssetModel *model = photos[i];
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != MYHTZAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = TIMTZScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < TIMTZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = TIMAssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * TIMTZScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(MYHTZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[MYHTZImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
//    }];
    return  [[PHImageManager defaultManager] requestImageDataForAsset:asset options:option resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
             ...
             NSString *filePaht = @"";
             if (imageData) {
                // 原图此处暂时固定的jpg 无法判断类型 gif图现在不发送原图数据
                if (@available(iOS 11, *)) {
                     if (asset.playbackStyle == PHAssetPlaybackStyleLivePhoto) {
                         CIImage *ciImage = [CIImage imageWithData:imageData];
                         CIContext *context = [CIContext context];
                         imageData = [context JPEGRepresentationOfImage:ciImage colorSpace:ciImage.colorSpace options:@{}];
                     }
                }
                filePaht = [[ICMediaManager sharedManager] saveImageData:imageData msgId:[kitUtils getMsgUUID] picType:@"jpg"];
                NSLog(@"选择的原照片路径=%@ 大小=%ld",filePaht,(long)[imageData length]);
             }
             ...
             if (!cancelled && result) {
                   ...
                   NSMutableDictionary *mutableDic =[NSMutableDictionary dictionaryWithDictionary:info];
                    [mutableDic setValue:filePaht forKey:@"OriginalPhotoPath"];
                   if (completion) completion(result,[mutableDic copy],isDegraded);
             }
...
- (MYHTZAlbumModel *)modelWithResult:(PHFetchResult *)result collection:(PHAssetCollection *)collection isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets options:(PHFetchOptions *)options {
    MYHTZAlbumModel *model = [[MYHTZAlbumModel alloc] init];
...
- (MYHTZAssetModel *)createModelWithAsset:(PHAsset *)asset {
    MYHTZAssetModelMediaType type = [[MYHTZImageManager manager] getAssetType:asset];
    NSString *timeLength = type == MYHTZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[MYHTZImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    MYHTZAssetModel *model = [MYHTZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//    return [NSNumber numberWithBool:![MYHTZImageManager manager].sortAscendingByModificationDate];
//}
//
//@end

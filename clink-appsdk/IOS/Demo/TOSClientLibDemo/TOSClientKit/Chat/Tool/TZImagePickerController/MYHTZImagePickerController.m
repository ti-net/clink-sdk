...
@interface MYHTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@implementation MYHTZImagePickerController
...
//    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:(83/255.0) green:(179/255.0) blue:(17/255.0) alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor grayColor];//[UIColor colorWithRed:(83/255.0) green:(179/255.0) blue:(17/255.0) alpha:0.5];
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[MYHTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[MYHTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<MYHTZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<MYHTZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<MYHTZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    MYHTZAlbumPickerController *albumPickerVc = [[MYHTZAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[MYHTZImageManager manager] authorizationStatusAuthorized]) {
            _tipLabel = [[UILabel alloc] init];
            _tipLabel.frame = CGRectMake(8, 120, self.view.tos_tz_width - 16, 60);
            ...
            NSString *tipText = [NSString stringWithFormat:[NSBundle tos_tz_localizedStringForKey:@"Allow %@ to access your album in \"Settings -> Privacy -> Photos\""],appName];
            _tipLabel.text = tipText;
            [self.view addSubview:_tipLabel];
            
            _settingBtn = [UIButton buttonWithType:UIButtonTypeSystem];
            [_settingBtn setTitle:self.settingBtnTitleStr forState:UIControlStateNormal];
            _settingBtn.frame = CGRectMake(0, 180, self.view.tos_tz_width, 44);
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    MYHTZPhotoPreviewController *previewVc = [[MYHTZPhotoPreviewController alloc] init];
...
- (void)configDefaultSetting {
    ...
    CGFloat cropViewWH = MIN(self.view.tos_tz_width, self.view.tos_tz_height) / 3 * 2;
    self.cropRect = CGRectMake((self.view.tos_tz_width - cropViewWH) / 2, (self.view.tos_tz_height - cropViewWH) / 2, cropViewWH, cropViewWH);
}
...
- (void)configDefaultBtnTitle {
    self.doneBtnTitleStr = [NSBundle tos_tz_localizedStringForKey:@"Done"];
    self.cancelBtnTitleStr = [NSBundle tos_tz_localizedStringForKey:@"Cancel"];
    self.previewBtnTitleStr = [NSBundle tos_tz_localizedStringForKey:@"Preview"];
    self.fullImageBtnTitleStr = [NSBundle tos_tz_localizedStringForKey:@"Full image"];
    self.settingBtnTitleStr = [NSBundle tos_tz_localizedStringForKey:@"Setting"];
    self.processHintStr = [NSBundle tos_tz_localizedStringForKey:@"Processing..."];
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[MYHTZImageManager manager] authorizationStatusAuthorized]) {
        ...
        MYHTZAlbumPickerController *albumPickerVc = (MYHTZAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[MYHTZAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        MYHTZPhotoPickerController *photoPickerVc = [[MYHTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[MYHTZImageManager manager] getCameraRollAlbumWithFetchAssets:NO completion:^(MYHTZAlbumModel *model) {
...
- (UIAlertController *)showAlertWithTitle:(NSString *)title {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:[NSBundle tos_tz_localizedStringForKey:@"OK"] style:UIAlertActionStyleDefault handler:nil]];
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
    ...
    self.cropRect = CGRectMake(self.view.tos_tz_width / 2 - circleCropRadius, self.view.tos_tz_height / 2 - _circleCropRadius, _circleCropRadius * 2, _circleCropRadius * 2);
}
...
- (void)setCropRect:(CGRect)cropRect {
    ...
    _cropRectLandscape = CGRectMake((self.view.tos_tz_height - widthHeight) / 2, cropRect.origin.x, widthHeight, widthHeight);
}

- (CGRect)cropRect {
    CGFloat screenHeight = [UIScreen mainScreen].bounds.size.height;
    BOOL isFullScreen = self.view.tos_tz_height == screenHeight;
    if (isFullScreen) {
        return _cropRect;
    } else {
        CGRect newCropRect = _cropRect;
        newCropRect.origin.y -= ((screenHeight - self.view.tos_tz_height) / 2);
        return newCropRect;
    }
}
...
- (void)setPickerDelegate:(id<MYHTZImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [MYHTZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [MYHTZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [MYHTZImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        MYHTZAssetModel *model = [MYHTZAssetModel modelWithAsset:asset type:[[MYHTZImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [MYHTZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(MYHTZAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
}

- (void)removeSelectedModel:(MYHTZAssetModel *)model {
...
- (UIImage *)createImageWithColor:(UIColor *)color size:(CGSize)size radius:(CGFloat)radius {
    ...
    if (size.width <= 0 || size.height <= 0 || isnan(size.width) || isnan(size.height)) {
...
- (void)viewDidLayoutSubviews {
    ...
    _progressHUD.frame = CGRectMake(0, progressHUDY, self.view.tos_tz_width, self.view.tos_tz_height - progressHUDY);
    _HUDContainer.frame = CGRectMake((self.view.tos_tz_width - 120) / 2, (_progressHUD.tos_tz_height - 90 - progressHUDY) / 2, 120, 90);
...
@interface MYHTZAlbumPickerController ()<UITableViewDataSource, UITableViewDelegate, PHPhotoLibraryChangeObserver> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation MYHTZAlbumPickerController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (imagePickerVc.allowPickingImage) {
        self.navigationItem.title = [NSBundle tos_tz_localizedStringForKey:@"Photos"];
    } else if (imagePickerVc.allowPickingVideo) {
        self.navigationItem.title = [NSBundle tos_tz_localizedStringForKey:@"Videos"];
    }
    
    if (self.isFirstAppear && !imagePickerVc.navLeftBarButtonSettingBlock) {
        self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle tos_tz_localizedStringForKey:@"Back"] style:UIBarButtonItemStylePlain target:nil action:nil];
    }
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[MYHTZImageManager manager] getAllAlbumsWithFetchAssets:!self.isFirstAppear completion:^(NSArray<MYHTZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (MYHTZAlbumModel *albumModel in self->_albumArr) {
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[MYHTZAlbumCell class] forCellReuseIdentifier:@"MYHTZAlbumCell"];
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.navigationController.navigationBar.isTranslucent) {
        ...
        tableViewHeight = self.view.tos_tz_height - top;
    } else {
        tableViewHeight = self.view.tos_tz_height;
    }
    _tableView.frame = CGRectMake(0, top, self.view.tos_tz_width, tableViewHeight);
    MYHTZImagePickerController *imagePickerVc = (MYHTZImagePickerController *)self.navigationController;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MYHTZAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MYHTZAlbumCell"];
    if (@available(iOS 13.0, *)) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MYHTZPhotoPickerController *photoPickerVc = [[MYHTZPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    MYHTZAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
+ (UIImage *)tz_imageNamedFromMyBundle:(NSString *)name {
    NSBundle *imageBundle = [NSBundle tos_tz_imagePickerBundle];
...
+ (void)configBarButtonItem:(UIBarButtonItem *)item tzImagePickerVc:(MYHTZImagePickerController *)tzImagePickerVc {
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    _languageBundle = [NSBundle bundleWithPath:[[NSBundle tos_tz_imagePickerBundle] pathForResource:preferredLanguage ofType:@"lproj"]];
}

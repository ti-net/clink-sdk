...
@property (nonatomic, strong) MYHTZProgressView *progressView;
@property (nonatomic, assign) int32_t bigImageRequestID;
@end

@implementation MYHTZAssetCell
...
- (void)setModel:(MYHTZAssetModel *)model {
    ...
    int32_t imageRequestID = [[MYHTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tos_tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[MYHTZImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(MYHTZAssetCellType)type {
    _type = type;
    if (type == MYHTZAssetCellTypePhoto || type == MYHTZAssetCellTypeLivePhoto || (type == MYHTZAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == MYHTZAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.tos_tz_left = self.videoImgView.tos_tz_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == MYHTZAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.tos_tz_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView tos_showOscillatoryAnimationWithLayer:_selectImageView.layer type:TZOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[MYHTZImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
    ...
    if (_model.type == MYHTZAssetCellTypeVideo) {
...
- (void)reload:(NSNotification *)noti {
    MYHTZImagePickerController *tzImagePickerVc = (MYHTZImagePickerController *)noti.object;
...
- (MYHTZProgressView *)progressView {
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.tos_tz_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.tos_tz_width - 27, 3, 24, 24);
    ...
    CGFloat progressXY = (self.tos_tz_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.tos_tz_height - 17, self.tos_tz_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.tos_tz_right, 0, self.tos_tz_width - self.videoImgView.tos_tz_right - 5, 17);
...
@interface MYHTZAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation MYHTZAlbumCell
...
- (void)setModel:(MYHTZAlbumModel *)model {
    _model = model;
    ...
    [[MYHTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.tos_tz_width - 24, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.tos_tz_height - titleHeight) / 2, self.tos_tz_width - 80 - 50, titleHeight);
...
@implementation MYHTZAssetCameraCell

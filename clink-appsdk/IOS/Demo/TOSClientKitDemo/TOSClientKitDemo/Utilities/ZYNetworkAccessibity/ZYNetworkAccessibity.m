...
- (void)showNetworkRestrictedAlert {
    if (self.alertController.presentingViewController == nil && ![self.alertController isBeingPresented]) {
        [[AppDelegate App].window.rootViewController presentViewController:self.alertController animated:YES completion:nil];
    }
}
...
- (UIAlertController *)alertController {
    if (!_alertController) {
        ...
        [_alertController addAction:[UIAlertAction actionWithTitle:@"前往" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            [self hideNetworkRestrictedAlert];
        }]];

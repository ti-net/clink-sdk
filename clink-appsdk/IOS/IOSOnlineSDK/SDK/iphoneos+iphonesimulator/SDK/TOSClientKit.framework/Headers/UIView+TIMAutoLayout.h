 ...
 * 用法示例：https://github.com/gsdios/TIMAutoLayout/blob/master/README.md
 *
 *********************************************************************************
...
@class TIMAutoLayoutModel, TIMUIViewCategoryManager;

typedef TIMAutoLayoutModel *(^MarginToView)(id viewOrViewsArray, CGFloat value);
typedef TIMAutoLayoutModel *(^Margin)(CGFloat value);
typedef TIMAutoLayoutModel *(^MarginEqualToView)(UIView *toView);
typedef TIMAutoLayoutModel *(^WidthHeight)(CGFloat value);
typedef TIMAutoLayoutModel *(^WidthHeightEqualToView)(UIView *toView, CGFloat ratioValue);
typedef TIMAutoLayoutModel *(^AutoHeight)(CGFloat ratioValue);
typedef TIMAutoLayoutModel *(^SameWidthHeight)(void);
typedef TIMAutoLayoutModel *(^Offset)(CGFloat value);
typedef void (^SpaceToSuperView)(UIEdgeInsets insets);

@interface TIMAutoLayoutModel : NSObject
...
@interface UIView (TIMAutoHeightWidth)
...
@property (nonatomic, readonly) TIMUIViewCategoryManager *sd_categoryManager;
...
@interface UIView (TIMLayoutExtention)
...
@interface UIView (TIMAutoLayout)

/** 开始自动布局  */
- (TIMAutoLayoutModel *)sd_layout;

/** 清空之前的自动布局设置，重新开始自动布局(重新生成布局约束并使其在父view的布局序列数组中位置保持不变)  */
- (TIMAutoLayoutModel *)sd_resetLayout;

/** 清空之前的自动布局设置，重新开始自动布局(重新生成布局约束并添加到父view布局序列数组中的最后一个位置)  */
- (TIMAutoLayoutModel *)sd_resetNewLayout;
...
- (void)addAutoLayoutModel:(TIMAutoLayoutModel *)model;
@property (nonatomic) TIMAutoLayoutModel *ownLayoutModel;
...
@interface UIScrollView (TIMAutoContentSize)
...
@interface UILabel (TIMLabelAutoResize)
...
@interface UIButton (TIMExtention)
...
@interface TIMAutoLayoutModelItem : NSObject
...
@interface UIView (TIMChangeFrame)
...
@interface TIMUIViewCategoryManager : NSObject

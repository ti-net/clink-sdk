 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `TIMModel` protocol for custom methods.
 
 
 Sample Code:
    
     ********************** json convertor *********************
     @interface TIMAuthor : NSObject
     @property (nonatomic, strong) NSString *name;
     @property (nonatomic, assign) NSDate *birthday;
     @end
     @implementation TIMAuthor
     @end
 
     @interface TIMBook : NSObject
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) NSUInteger pages;
     @property (nonatomic, strong) TIMAuthor *author;
     @end
     @implementation TIMBook
     @end
    
     int main() {
         // create model from json
         TIMBook *book = [TIMBook yy_modelWithJSON:@"{\"name\": \"Harry Potter\", \"pages\": 256, \"author\": {\"name\": \"J.K.Rowling\", \"birthday\": \"1965-07-31\" }}"];
     ...
     ********************** Coding/Copying/hash/equal *********************
     @interface TIMShadow :NSObject <NSCoding, NSCopying>
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) CGSize size;
     @end
 
     @implementation TIMShadow
 ...
 */
@interface NSDictionary (TIMModel)
...
/**
 ...
 There's no need to add '<TIMModel>' to your class header.
 */
@protocol TIMModel <NSObject>
@optional
...
/**
 ...
 object will be add to the array/set/dictionary.
  ...
  Example:
        @class TIMShadow, TIMBorder, TIMAttachment;
 
        @interface TIMAttributes
        @property NSString *name;
        ...
        @implementation TIMAttributes
        + (NSDictionary *)modelContainerPropertyGenericClass {
            return @{@"shadows" : [TIMShadow class],
                     @"borders" : TIMBorder.class,
...
/**
 ...
 Example:
        @class TIMCircle, TIMRectangle, TIMLine;
 
        @implementation TIMShape

        + (Class)modelCustomClassForDictionary:(NSDictionary*)dictionary {
            if (dictionary[@"radius"] != nil) {
                return [TIMCircle class];
            } else if (dictionary[@"width"] != nil) {
                return [TIMRectangle class];
            } else if (dictionary[@"y2"] != nil) {
                return [TIMLine class];
            } else {
                return [self class];
            }
        }
